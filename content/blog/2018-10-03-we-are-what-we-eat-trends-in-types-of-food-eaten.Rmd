...
filenames = dir(path = "../../static/data/diet/supp_data/servings.xls", pattern="*.xls")
  
  sheetnames <- excel_sheets("../../static/data/diet/supp_data/servings.xls")
  
  servings_wkbk <- lapply(excel_sheets("../../static/data/diet/supp_data/servings.xls"), read_xls, path = "../../static/data/diet/supp_data/servings.xls", range = "A7:H53", na = c("","7/","9/","NA","N/A",""))
  
  # name the dataframes using the sheetnames
  names(servings_wkbk) <- sheetnames
  
  #keep only totals sheet
  servings <- as.data.frame(servings_wkbk[(names(servings_wkbk) %in% c('Totals'))])
  

  names(servings) <- c('year','protein_oz_daily','dairy_cups_daily','fruit_cups_daily','veg_cups_daily',
                     'grain_oz_daily','fat_oil_g_daily','addedsugar_tsp_daily')

  
##### recommended servings
  
  filenames = dir(path = "../../static/data/diet/supp_data/nutrients_recommended.xlsx", pattern="*.xlsx")
  
  sheetnames <- excel_sheets("../../static/data/diet/supp_data/nutrients_recommended.xlsx")
  
  rec_servings_wkbk <- lapply(excel_sheets("../../static/data/diet/supp_data/nutrients_recommended.xlsx"), read_xlsx, path = "../../static/data/diet/supp_data/nutrients_recommended.xlsx", range = "A2:O40", na = c("","7/","9/","NA","N/A",""))
  
  # name the dataframes using the sheetnames
  names(rec_servings_wkbk) <- sheetnames
  
  #keep only servings sheet
  rec_servings <- as.data.frame(rec_servings_wkbk[(names(rec_servings_wkbk) %in% c('food_groups'))])
  
  names(rec_servings) <- rec_servings[1,]
  
  rec_servings <- rec_servings[-1,]
  rec_servings <- rec_servings[1:16,1:9]
  
  
  rec_nutrients <- as.data.frame(rec_servings_wkbk[(names(rec_servings_wkbk) %in% c('nutrients'))])
  names(rec_nutrients) <- rec_nutrients[1,]
  rec_nutrients <- rec_nutrients[-1,]
  
  
  names(rec_nutrients) <- c('nutrient','type','calories_1000','calories_1200','calories_1400',
                       'calories_1600_1','calories_1600_2','calories_1800','calories_2000',
                       'calories_2200','calories_2400','calories_2600','calories_2800',
                       'calories_3000','calories_3200')
  
  
  
  rec_servings_2000 <- rec_servings[,c('food_group','units','calories_2000')]
  servings2 <- melt(servings, id=c("year"))
  names(servings2) <- c('year','food_group','serving')
  
  servings2 <- servings2 %>% mutate(label = ifelse(food_group == 'protein_oz_daily', 'Protein Foods',
                                            ifelse(food_group == 'fruit_cups_daily', 'Fruits',
                                            ifelse(food_group == 'veg_cups_daily', 'Vegetables',
                                            ifelse(food_group == 'grain_oz_daily', 'Grains',
                                            ifelse(food_group == 'dairy_cups_daily', 'Dairy',
                                                                        'Other'))))))
  
  servings2 <- servings2[servings2$label != 'Other',]
  servings3 <- servings2[,-2]
  
  rec_servings_2000$year <- '2016'
  rec_servings_2000 <- rec_servings_2000[,-2]
  names(rec_servings_2000) <- c('label','serving','year')
  rec_servings_2000 <- rec_servings_2000[,c(3,2,1)]
  rec_servings_2000 <- rec_servings_2000[rec_servings_2000$label %in% c('Protein Foods','Fruits','Vegetables','Grains','Dairy'),]
  servings_df <- rbind(servings3, rec_servings_2000)
  servings_df$serving <- as.numeric(servings_df$serving)
  
  servings_df_index <- servings_df %>%
    group_by(label) %>%
    mutate(index_daily_unit = round(serving/serving[year==2016],3))
  
  servings_df_index <- servings_df_index[servings_df_index$year != 2016,]
  servings_df_index$value_label <- paste(servings_df_index$index_daily_unit*100,'%',sep='')
  
  servings_df_index$year <- as.numeric(servings_df_index$year)
  servings_df_index <- servings_df_index[servings_df_index$year %in% c(1970,1975,1980,1985,1990,1995,2000,2005,2010,2015),]
  
  
  
##### actual nutrients 
  
  filenames = dir(path = "../../static/data/diet/supp_data/nutrients.xls", pattern="*.xls")
  
  sheetnames <- excel_sheets("../../static/data/diet/supp_data/nutrients.xls")
  
  nutrients_wkbk <- lapply(excel_sheets("../../static/data/diet/supp_data/nutrients.xls"), read_xls, path = "../../static/data/diet/supp_data/nutrients.xls", range = "A6:AD108", na = c("","7/","9/","NA","N/A",""))
  
  # name the dataframes using the sheetnames
  names(nutrients_wkbk) <- sheetnames
  
  #keep only servings sheet
  nutrients <- as.data.frame(nutrients_wkbk[(names(nutrients_wkbk) %in% c('Totals'))])
  
  #names(nutrients) <- nutrients[1,]
  
  #nutrients <- nutrients[-1,]

  names(nutrients) <- c('year','food_energy_cal','carbs_g','protein_g','total_fat_g','saturated_fat_g',
                        'mono_fat_g','poly_fat_g','cholesterol_mg','fiber_g','vitamin_a_re_microg',
                        'vitamin_a_rae_microg','carotene_microg','vitamin_e_mg_alphate','vitamin_c_mg',
                        'thiamin_mg','riboflavin_mg','niacin_mg','vitamin_b6_mg','folate_microg',
                        'vitamin_b12_microg','calcium_mg','phosphorus_mg','magnesium_mg','iron_mg',
                        'zinc_mg','copper_mg','potassium_mg','selenium_microg','sodium_mg')
  
  
  
  rec_nutrients <- rec_nutrients[,c("nutrient", 'calories_2000')]
  
  nutrients2 <- melt(nutrients, id=c("year"))
  names(nutrients2) <- c('year','nutrient','availability')
  
  nutrients2 <- nutrients2 %>% mutate(label = ifelse(nutrient == 'total_fat_g', 'Total lipid (fat), g',
                                              ifelse(nutrient == 'cholesterol_mg', 'Cholesterol, mg',       
                                              ifelse(nutrient == 'vitamin_a_rae_microg', 'Vitamin A,  mg_RAE',
                                              ifelse(nutrient == 'vitamin_c_mg', 'Vitamin C, mg',
                                              ifelse(nutrient == 'vitamin_b6_mg', 'Vitamin B-6, mg',
                                              ifelse(nutrient == 'vitamin_b12_microg', 'Vitamin B-12, mg',
                                              ifelse(nutrient == 'calcium_mg', 'Calcium, mg',
                                              ifelse(nutrient == 'iron_mg', 'Iron, mg',
                                              ifelse(nutrient == 'potassium_mg', 'Potassium, mg',
                                              'Other'))))))))))
                                                     
  
 nutrients2 <- nutrients2[nutrients2$label != 'Other',]
 nutrients3 <- nutrients2[,-2]
 nutrients3$availability <- as.numeric(as.character(nutrients3$availability))
  
  
 rec_nutrients$year <- 2011
 names(rec_nutrients) <- c('label','availability','year')
 rec_nutrients <- rec_nutrients[,c(3,2,1)]
 rec_nutrients <- rec_nutrients[rec_nutrients$label %in% c('Total lipid (fat), g','Cholesterol, mg','Vitamin A,  mg_RAE','Vitamin C, mg','Vitamin B-6, mg',
                                                           'Vitamin B-12, mg','Calcium, mg','Iron, mg','Potassium, mg'),]
 
 
 nutrients_df <- rbind(nutrients3, rec_nutrients)

 nutrients_df$label <- as.factor(nutrients_df$label)
 
 nutrients_df_index <- nutrients_df %>%
   ungroup() %>%
   group_by(label) %>%
   mutate(index_daily_unit = round(availability/availability[year==2011],3))
 
 nutrients_df_index <- nutrients_df_index[nutrients_df_index$year != 2011,]
 nutrients_df_index$value_label <- paste(nutrients_df_index$index_daily_unit*100,'%',sep='')
 
 nutrients_df_index$year <- as.numeric(nutrients_df_index$year)
 #nutrients_df_index <- nutrients_df_index[nutrients_df_index$year %in% c(1970,1975,1980,1985,1990,1995,2000,2005,2010,2015),]
 
 #nutrients_df_index <- nutrients_df_index[!(nutrients_df_index$label %in% c('Total lipid (fat), g','Cholesterol, mg')),] 
 nutrients_df_index <- nutrients_df_index[nutrients_df_index$year != 1909,]

  
```
  

Inspired by the articles I linked in my [previous post,](https://www.datamadepretty.com/blog/we-are-what-we-eat-getting-the-data-into-r/) ([here](http://www.pewresearch.org/fact-tank/2016/12/13/whats-on-your-table-how-americas-diet-has-changed-over-the-decades/), [here](https://www.vox.com/science-and-health/2017/2/2/14485226/americans-avocado-consumption-usda-report?mc_cid=dba0fc78bc&mc_eid=8f698ac693), and [here](https://cspinet.org/resource/changing-american-diet-0)), I decided to investigate the USDA's food consumption dataset to get an idea of how Americans' diets have changed over time.  I am also complementing this dataset with Harris Poll and Google Trends data.  In the previous post, I demonstrated how I read the Excel files containing the USDA data into R and scraped the Harris Poll HTML tables.  In this post, I am going to be sharing some interesting findings derived from my data.
...
ggplot(data=diverse_food_index, aes(x=year, y=index_percapita_lbyr, color = foodtype)) +
  ...
  scale_x_continuous(name = "Year", breaks=seq(1970,2015,5))+
  scale_y_continuous(labels = percent, name = "% Growth in Pounds per Capita Since 1970*")+
...
ggplot() +
  ...
  scale_x_continuous(name = "Year", breaks=seq(1970,2015,5))+
  scale_y_continuous(labels = percent, name = "% Growth in Pounds Imported Since 1970*")+
...
harris_1a_crop_2 <- harris_1a_crop_2 %>% mutate(label = ifelse(rating == 'important_net', 'Important (Total)',
                                                        ifelse(rating == 'not_important_net', 'Not Important (Total)',
                                                               'Not At All Sure')))

harris_1a_crop_2$percent <- as.numeric(as.character(harris_1a_crop_2$percent))

harris_1a_crop_2 <- harris_1a_crop_2 %>%
  mutate(label =  factor(label, levels = c('Not At All Sure','Not Important (Total)','Important (Total)'))) %>%
  arrange(label)

ggplot(data = harris_1a_crop_2, aes(x = fct_inorder(purchase_factor), y = percent, fill = label)) + 
  geom_bar(stat = 'identity') + 
  scale_fill_manual(values = c("Important (Total)" = "plum", "Not Important (Total)" = "skyblue", "Not At All Sure" = "palegreen3"))+
  coord_flip()+
  ...
  geom_text(data = harris_1a_crop_2[harris_1a_crop_2$label == 'Important (Total)',], aes(x= fct_inorder(purchase_factor) ,y=percent,
            label = paste(percent,'%',sep="")), size = 3.5,
            position = 'stack',hjust = 2 )+
  geom_text(data = harris_1a_crop_2[harris_1a_crop_2$label == 'Not Important (Total)',], aes(x= fct_inorder(purchase_factor) ,y=100-percent,
            label = paste(percent,'%',sep="")), size = 3.5,
...
ggplot(data = harris_1b_crop_equal, aes(x = purchase_factor, y = percent_important, fill = label)) + 
  geom_bar(stat = 'identity') + 
  scale_y_continuous(name = "% Answering Important (Total)", breaks = NULL)+
  scale_x_discrete(name = "Purchase Factor")+
...
ggplot(harris_1b_children, aes(x=reorder(purchase_factor,differential), y=differential, label=paste(differential,'%',sep = ''))) + 
  geom_point(stat='identity', aes(color=ifelse(differential > 0, 'olivedrab', 'firebrick2')), size=10)  +
  ...
  scale_x_discrete(name = "Purchase Factor") +
  scale_y_continuous(name = "% Differential in Answering Important (Total) Between HHs With and Without Children")+
  theme_bw()+
  theme(axis.title.x = element_text(colour="black", size=12),
        axis.title.y = element_text(colour="black", size=12),
...
###An Increase in Allergies Shapes Menus for All
...
Americans are not eating the correct amount of each food group.  This was the case in 1970 and remains the case today.  For example, the [2015 USDA Recommended Healthy Eating Pattern Guidelines](https://health.gov/dietaryguidelines/2015/guidelines/appendix-3/) outline that Americans who consume a 2,000 calorie diet should be eating 2.5 cups of vegetables per day.  In 1970, on average, Americans only ate 1.5 cups per day and 45 years later, in 2015, the number had only grown to 1.7 cups per day.  The situation is even worse for fruit.  While the USDA Guidelines suggest 2 cups per day, the average American in 1970 only ate 0.7 cups per day.  In 2015, that number still stood way below recommended at 0.8 cups per day.  On the other hand, Americans have been and still are [eating way too much protein](https://www.bloomberg.com/news/articles/2016-11-04/enough-with-the-protein-america) and grains.  Grain consumption has been above 100% of what the USDA Guidelines recommend since the 1990s as is seen in the graph below.  Protein consumption was always more than 100% of the recommended amount, and has only grown more recently.  Surprisingly, Americans' consumption of dairy hasn't changed much over time since the decline in milk consumption has been replaced with a one for one increase in cheese and yogurt consumption.


```{r warning=FALSE, message=FALSE, eval=TRUE, echo=FALSE, fig.width=9,fig.height=6,fig.align='center'}  
  ggplot(data = servings_df_index, aes(x = label, y = index_daily_unit, fill = label)) + 
    geom_bar(stat = 'identity',position="dodge") + 
    geom_hline(yintercept = 1, linetype = 'dashed')+
    scale_y_continuous(name = "\n Percent of Recommended Daily Servings",labels = percent)+
    scale_x_discrete(name = NULL)+
    #guides(fill=guide_legend(title="Food Groups"))+
    guides(fill=FALSE)+
    labs(title = "Americans Overeat Protein and Grains While Undereating Fruits and Veggies: {closest_state}")+
    geom_text(data = servings_df_index, aes(x = label, y = index_daily_unit, group = label, label = value_label),
              nudge_y = .1,size = 4)+
    theme_bw()+
    theme(axis.text.x = element_text(size=12),
          ...
          plot.title = element_text(size = 14,colour="deepskyblue4"))+
    transition_states(year, 1, 3, wrap = FALSE) +
...
nutrients_df_index <- nutrients_df_index[nutrients_df_index$year >= 1970,]
nutrients_df_index$imaginary <- FALSE

#creating fake data to show stationary trend

nutrients_df_imaginary <- nutrients_df_index[FALSE,]

years <- c(2011:2025)

for(year in years){
nutrients_imaginary <- nutrients_df_index[nutrients_df_index$year == 2010,]
nutrients_imaginary$year <- year
nutrients_df_imaginary <- rbind(nutrients_df_imaginary,nutrients_imaginary)
}

#rbind imaginary to first df
nutrients_df_index <- rbind(nutrients_df_index, nutrients_df_imaginary)
nutrients_df_index[nutrients_df_index$year > 2010,]$imaginary <- TRUE


 ggplot(data=nutrients_df_index, aes(x=year, y=index_daily_unit, color = label)) +
  geom_line(data = filter(nutrients_df_index, !imaginary), linetype = "solid") + 
  geom_line(data = filter(nutrients_df_index, imaginary), linetype = "dashed")+
  #geom_label(data = nutrients_df_index, aes(x=2025, y=index_daily_unit[index_daily_unit$year==2025,], label = '?'), show.legend = F)+
   guides(color=guide_legend(title="Nutrients", override.aes = list(size=2)))+
   scale_x_continuous(name = "Year", breaks=seq(1970,2025,5))+
   scale_y_continuous(labels = percent, name = "Availability of Nutrient as % of Recommended Daily Value")+
   ggtitle("Availability of Fat Remains Very High, While Vitamins Remains Mostly Good")+
  theme_bw()+
      theme(axis.text.x = element_text(size=12),
          axis.text.y = element_text(size=12),
          axis.title.y = element_text(size=12),
          plot.title = element_text(size = 14,colour="deepskyblue4"))+
   transition_reveal(label, year)
